...
for i in np.arange(len(posx)):
    ...
    plt.plot(np.round(posx[i]/div)*10,np.round(posy[i]/div)*10,linewidth = 0, color = 'blue',marker='x')
plt.show()

#Now we define interpolation scheme and distriubte charge
def charge():
...
#Distribute E field:
def E(i,j):
    E[i][j] = (phi[i-1][j]-phi[i+1][j])/(2*div) + (phi[i][j-1]-phi[i][j+1])/(2*div)
