...
# NOWW with LAZERS!
#
# Made longer gas channel
#
# It is clear we need more particles
# It is clear that PP has been pushed to its limits
# On to the next tier of simulation.
...
Np = 170 #number of particles, 150 is max that works well

X = random.rand(Np)*45 - 12.5 #Assigning random x values to particles
...
def setlattice(n):
    for i in np.arange(1,n):
        ...
        Q[i] = 1
        X[i] = -15 + 0.65*i
        Y[i] = np.round(random.rand()*25)-12.5
setlattice(90)     
...
for i in np.arange(tmax): #updates time, resets arrays, calls required functions
    ...
    #plt.plot(X,Y,marker = 'o',linewidth = 0,ms=50,color = 'blue',alpha = 200)
